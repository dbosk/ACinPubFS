    ...
    \begin{minipage}{0.96\textwidth}
      \begin{algorithmic}
        \Function{$\SplitGroupOp$}{$S_0, S_1$}
          ...
          \State{%
            $i_0\rgets \{0, 1\}^\lambda$, $i_1\rgets \{0, 1\}^\lambda$
          }
          \Comment{Generate outboxes, one per new group.}
          \State{%
          ...
          \For{$s \in S$}
            \If{$s\in S_0$}
              \State{%
                $c_{s}\gets{ \Enc[\PubKey{s}]{i_0, \VerifKey{}}}$
              }
            \Else\Comment{$s\in S_1$}
              \State{%
                $c_{s}\gets{ \Enc[\PubKey{s}]{i_1, \VerifKey{}}}$
              }
            \EndIf{}
          \EndFor{}
          ...
          \State{%
            \Return{$i_0, i_1, (\VerifKey{}, C, \sigma)$}
          }
          \Comment{\Iacs{ANOBE} ciphertext.}
        \EndFunction{}
      \end{algorithmic}
    \end{minipage}
  }
...
We now have a privacy-preserving publication protocol.
In its current state no subscribers can verify the authenticity of any message.
Eve could simply replace a ciphertext with any other: she has all public keys 
and the verification key in the ciphertext is one-time and ciphertext specific.

We can easily achieve authenticity by substituting \(i_p\) for \((i_p, 
  \MACKey{p})\) and then compute a \ac{MAC} for every ciphertext.
This allows any subscriber to publish an authenticated message as well.
If we do not desire this property we can replace the \ac{MAC} by digital 
signatures: we get \((i_p, \SignKey{p})\) on the publisher side, \((i_p, 
  \VerifKey{p})\) on the subscriber side.
This also means that \(\psplit{p}\) will generate a new \ac{MAC} key or 
signature-verification key-pair too.
