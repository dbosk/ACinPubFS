...
We treat this problem in \cref{UC}, until then we will refer to this as 
\enquote{the problem of Jealous Bob}.
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \begin{algorithmic}
      ...
      \Function{$\pnget{s}$}{}
        ...
        \State{$M\gets \emptyset$}
        \For{$\sigma\in C$}
          \State{$(i_s^\prime, m_\sigma) \gets \Dec[\PriKey{s}, 
            \VerifKey{p}]{\sigma}$}
          \State{$M\gets M\cup \{m\}$}
          \State{$C^\prime \gets \fsread[i_s^\prime]$}
...
\section{Universally Composable Security}
\label{UC}
