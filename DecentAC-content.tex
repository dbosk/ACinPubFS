...
\section{Analysis of the Pull Model}
\label{PullAnalysis}
    ...
    \begin{minipage}{0.96\textwidth}
      \begin{algorithmic}
        \Function{$\SplitGroupOp$}{$S_0, S_1$}
          ...
          \State{%
            $(\SignKey{}, \VerifKey{})\gets{\Keygen{\lambda}}$
          }
          \Comment{%
...
\begin{definition}[Split-group indistinguishability]\label{SplitGroupSecurity}
  ...
  \begin{description}
    ...
    \item[Challenge] The adversary gives the challenger a set of uncorrupted 
      ...
      The challenger randomly chooses \(B = (B_0, \ldots, B_n) \in \{0, 1\}^n\) 
      and constructs \(R_0, R_1\) as follows:
...
Alice posting to two different recipient sets and Alice posting to one and 
Carol to the other.
...
\begin{definition}[Extended Pull Model]
  \dots
\end{definition}


\section{Analysis of the Push Model}
\label{PushAnalysis}
...
ciphertexts.
More formally, we break random permutaion
