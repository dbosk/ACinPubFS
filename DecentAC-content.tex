...
\begin{definition}[Communication model]\label{CommunicationModel}
  ...
  \begin{itemize}
    ...
    \item The publisher \(p\) uses \(\cpub[p, S]{p}[m]\) to publish the message 
      \(m\) to all subscribers \(S\).

    \item Each subscriber \(s\in S\) uses \(\cget[p, S]{s}[]\) to get the set 
      \(M\) of published messages \(m\in M\).

    \item The publisher can also use \(\csplit[p, S]{p}[S_0, S_1]\) to split 
      \(S\) into disjoint sets \(S_0\) and \(S_1\) and the corresponding 
      \(\C[p, S_0]\) and \(\C[p, S_1]\).
  \end{itemize}
\end{definition}
...
subscribers.
We note that no subscriber \(s\in S\) can use \(\cpub{s}[m]\) to publish 
a message \(m\).
...
It follows too that no subscriber \(s_i\) can learn which \(s_1, \ldots, s_n\in 
  S\) beyond that \(i\in \{1, \ldots, n\}\) and \(s_i\in S\) for a given 
message \(m\).
We will treat this more formally when we analyse the implementations.
