...
\cref{PushAndPull}.
The we give two models implementing that functionality and analyse what 
properties their primitives must have in \cref{PullAnalysis,PushAnalysis}.
Finally, in \cref{Composed}, we construct a protocol composed of several 
instances of either of the two models, then prove prove its security.
...
\section{An Anonymous File System}\label{FileSystem}
...
We will now introduce an anonymizing layer on top of the file system.

\begin{definition}[Anonymizing functionality]\label{AnonymizingFunctionality}
  We denote by \(\M\) an \emph{anonymizing functionality}.
  Then \(\M\) provides the interface \(\M[\cdot]\), which takes a function call
  and its arguments and executes the call anonymously.
\end{definition}

We note that \(\M\) can be implemented using a mix-net or onion-routing, it may 
arbitrary delay the execution of the function if necessary for the 
anonymization.
We summarize its capabilities in the following security game.

\begin{definition}[MIX-IND]
  Let \(\M\) be an anonymizing functionality.
  \begin{description}
    \item[Setup] \dots
    \item[Challenge] The adversary prepares two sets of standalone operations, 
      \(O_1, O_2\), and gives these to the challenger.
      The challenger then randomly chooses \(b\in \{0, 1\}\) and executes 
      \(\M[O_b]\) followed by \(\M[O_{1-b}]\).
    \item[Guess] The adversary observes the executions as they happen and then 
      outputs \(\hat{b}\).
  \end{description}
  We define the adversary's advantage as \(\Adv{\text{MIX-IND}}{\A, 
      \M}[\lambda] = |\Pr[\hat{b} = b] - \frac{1}{2}|\).
\end{definition}

Now, combining this anonymizing functionality with the public file system we 
get an anonymous public file system.

\begin{definition}[Anonymous public file system]\label{AFS}
  Let \(\FS\) be a public file system and \(\M\) be an anonymizing 
  functionality.
  Then we let \(\AFS\) be an anonymized file system with the following 
  interface:
  \begin{itemize}
    \item \(\afscreate[i][m] = \M[\fscreate[i][m]]\),
    \item \(\afsread[i] = \M[\fsread[i]]\), and
    \item \(\afsappend[i][m] = \M[\fsappend[i][m]]\).
  \end{itemize}
\end{definition}

Now, using \(\AFS\), Eve cannot distinguish between Alice's and Bob's requests 
to the operations of \(\FS\).
Eve can record the times at which they occurred with her own clock, since she 
executes them, but the anonymizing layer \(\M\) might render this useless --- 
since it can reorder and delay the requests for execution.
...
We will focus on two alternative protocols which uses \(\AFS\) from \cref{AFS} 
...
\begin{definition}[Pull Model]\label{PullModel}
  ...
  broadcast-encryption scheme and \(\AFS\) be an anonymized public file system.
...
notation, further, the proof in \cref{PushAnalysis} will be similar enough.)
...
index.
This actually reduces the problem to the Push Model which we will analyse 
next.
