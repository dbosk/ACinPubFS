...
\cref{PushAndPull}.
Then we will discuss the high-level properties in 
...
In this model, each publisher has an index instead of each subscriber having an 
inbox.
...
The set-up phase is similar as in the Push Model.
Alice (\(\psetup{p}\)) creates a signature-verification key-pair, but in 
addition, she randomly chooses an identifier for her index.
Her friends (\(\psetup{s}\)) now only create a public-private key-pair, they 
have no need for any identifier.
...
The operation \(\pget{s}\) works exactly the same as for the Push Model.

% XXX Write high-level analysis of pull model
Some high-level analysis \dots
...
index.
But we can see that this reduces the problem to the Push Model, except with 
broadcast ciphertexts instead.


\section{Security Proofs and Other Necessary Stuff}
