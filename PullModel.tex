...
\begin{definition}[Split-IND-CCA]\label{SplitGroupSecurity}
...
\begin{definition}[Pull-IND-CCA]
  We have the following game:
  \begin{description}
    \item[Setup] The challenger creates two instances \(\Pull[p_0, S_0]\) and 
      \(\Pull[p_1, S_1]\), then immediately runs \(\pnsplit[p_1, S_1]{p}[S_1^0, 
        S_1^1]\).
      The adversary is given the public keys and verification keys for all 
      \(s\in S_0\cup S_1^0\cup S_1^1\).
    ...
    \item[Phase 1] The adversary may request encryptions and decryptions for 
      ...
      However, the decryption oracle must not be used on the ciphertext written
      to the file system in the split.
    ...
    \item[Challenge] The adversary gives two messages \(m_0\) and \(m_1\) to 
      ...
      If \(b = 1\) the challenger will randomly choose two recipients \(r, 
        r^\prime\) from the sets \(S_1^i\), otherwise \(r\) from \(S_0\) and 
      \(r^\prime\) from \(S_1^0\cup S_1^1\).
    ...
    \item[Phase 2] The adversary is allowed to continue querying the oracles, 
      except for the challenge ciphertexts \(c\) and \(c^\prime\) and the split 
      ciphertext from the setup.

    \item[Guess] The adversary outputs \(\hat{b}\) and wins if \(\hat{b} = b\).
  \end{description}
  We define the advantage as \(\Adv{\text{Pull-IND-CCA}}{\A}[1^\lambda] 
    = |\Pr[\hat{b} = b] - \frac{1}{2}|\).
\end{definition}
...
\begin{theorem}
  The Pull Model (\cref{PullModel}) yields Pull-IND-CCA security if the 
  underlying encryption scheme is IND-CCA secure.
  More specifically,
  \begin{equation*}
    \Adv{\text{Pull-IND-CCA}}{\A}[1^\lambda]\leq \Adv{\text{IND-CCA}}{\A, 
      \mathcal{E}}[1^\lambda],
  \end{equation*}
...
\begin{proof}[sketch]
  ...
  a split, we can see in \cref{SplitGroup} that the two instances only depends 
  on the randomly chosen identifiers and verification keys, and the sets of 
  users.
  It follows that \(\Pull[p_1, S_1^0]\) is as independent from \(\Pull[p_1, 
    S_1^1]\) as \(\Pull[p_0, S_0]\) is independent of \(\Pull[p_1, S_1^i]\), 
  for
  \(i\in \{0, 1\}\).
  Thus the only way the adversary can win is by distinguishing \(m_0\) from 
  \(m_1\).
  \qed{}
\end{proof}
