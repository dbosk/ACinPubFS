...
\date{IFIP Summer School, 20th August 2015}
...
\begin{frame}<presentation>{Our Contributions}
  \begin{itemize}
    ...
    \item We achieve Hidden-Credentials, Hidden-Policies, and 
      Hidden-Decisions~\cite{TowardsPPACwHPHCHD} for our scheme.

    \item For this we use \ac*{ANOBE}~\cite{ANOBE}.
  \end{itemize}
\end{frame}
\mode<presentation>{\acused{ANOBE}}
...
\begin{frame}<presentation>{Anonymous Broadcast Encryption}
  ...
  \begin{itemize}
    ...
    \item \ac{ANOBE} does the same without the description of \(S\).
...
\begin{frame}
  \mode<presentation>{{ANOBE Encryption}}
  \begin{figure}
    \begin{algorithmic}[1]
...
\begin{frame}<presentation>{DOSN Architecture}
  \begin{itemize}
    \item The \ac{DOSN} architecture can be achieve in several ways.
    ...
    \item The Push Model is more like magazine subscriptions.
  \end{itemize}
\end{frame}
}
...
\mode<presentation>{%
  ...
  \begin{itemize}
    \item The space complexity for key management is similar for both models: 
      \emph{linear} in the number of friends.

      \pause{}

    \item The Pull Model has slightly higher (constant factor) due to the 
      tag-hint system.
...
\subsection<article>{Future Work}
